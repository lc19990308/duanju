	...
	<view class="page">
		...
		<view class="contentBox" v-if="contentShow" :style="{width:videoStyle.width,height:videoStyle.height}">
				...
				<swiper-item v-for="(item,index) in vodList" :key="index" class="box">
					<view :style="{opacity:shadeNum > 2 ?'0':'1',width:videoStyle.width,height:videoStyle.height}">
						...
						<view class="fullScreen" v-if="item.fullScreenShow" @click="openScreen(index)" :style="{left: (screenWidth/2) - 46.5 + 'px', bottom: vodHeight / 3.4 + 'px'}"
						 :class="[vodIndex == index ? (moveOpacity?'vodMenu-bright2':'vodMenu-bright0') : '']">
							<image src="/static/icon/tsp-qp.png" class="fullScreen-icon"></image>
			...
			<view class="flooter" :style="{opacity:moveOpacity ? 0 : 1,bottom:speedBottom+'px'}" v-if="vodList.length > 0 && beforeVodInfo.sliderShow">
				...
				<!-- 进度条 -->
				<view class="sliderBox" :style="{opacity:shadeNum > 2 ?'0':'1'}" @touchmove.stop="touchmoveSlider" @touchstart.stop="touchmoveSlider" @touchend.stop="touchendSlider" v-if="vodList.length > 0 && beforeVodInfo.sliderShow">
