	...
	<refresh class="refresh" @refresh="onrefresh" @pullingdown="onpullingdown" :display="refreshing ? 'show' : 'hide'"
		...
		<div class="refresh-view">
			<image class="refresh-icon" :src="refreshIcon"
				:style="{width: (refreshing || pulling) ? 0: '70rpx',color:fontColor}"
				:class="{'refresh-icon-active': refreshFlag}"></image>
			<uniloadmore v-if="refreshing" class="loading-icon" status="loading" :contentText="loadingMoreText"
				:style="{color:fontColor}"></uniloadmore>
	...
	export default {
		...
		data() {
			return {
				...
				language: {
				  zh_CN: {
				    pullToRefresh: '下拉可以刷新',
				    refreshing: '正在刷新...',
				    releaseToRefresh: '释放立即刷新',
				    refreshCompleted: '已刷新',
				  },
				  vi_VN: {
				    pullToRefresh: 'Kéo xuống để làm mới',
				    refreshing: 'Đang làm mới...',
				    releaseToRefresh: 'Thả ra để làm mới ngay',
				    refreshCompleted: 'Đã làm mới',
				  },
				  zh_EN: {
				    pullToRefresh: 'Pull down to refresh',
				    refreshing: 'Refreshing...',
				    releaseToRefresh: 'Release to refresh',
				    refreshCompleted: 'Refreshed',
				  }
				}
		...
		methods: {
			...
			onrefresh(e) { //下拉刷新释放刷新
				...
				this.refreshText = this.language[this.lang].refreshing;
				this.$emit("refresh", e);
			},
			onpullingdown(e) { //下拉刷新触发
				...
				if (Math.abs(e.pullingDistance) > Math.abs(e.viewHeight)) {
					...
					this.refreshText = this.language[this.lang].pullToRefresh;
				}
			...
			closeRefresh() {
				...
				this.refreshText = this.language[this.lang].refreshCompleted;
			}
		},
	}
</script>
